...
import { userHistoryColumns } from '../user-history-table-config';
...
export function UserHistoryTable({
  ...
  const fairteilerOptions = () => {
    const fairteilers = new Set<string>();
    data.forEach((item: vContribution) => {
      if (item.fairteilerName) {
        fairteilers.add(item.fairteilerName);
      }
    });
    return Array.from(fairteilers)
      .sort()
      .map((fairteiler) => ({
        label: fairteiler,
        value: fairteiler,
    ...
    <div className='w-full'>
      <div className='flex flex-1 flex-col flex-wrap gap-2 md:flex-row md:items-center md:gap-4'>
        ...
        <div className='flex flex-wrap gap-2'>
          {table.getColumn('fairteiler') && (
            <DataFilter
              column={table.getColumn('fairteiler')}
              title='Fairteiler'
              options={fairteilerOptions()}
            />
          )}
        ...
        <Table>
          ...
          <TableBody>
            {table.getPaginationRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell
                  colSpan={userHistoryColumns.length}

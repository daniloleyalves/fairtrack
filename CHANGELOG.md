...
- Refactor Phase 4d (REST cleanup — narrow scope): four route handlers with zero remaining callers deleted — `src/app/api/{user/gamification/milestone-progress,platform/contributions,platform/fairteilers,tutorial/progress}/route.ts` — plus their now-empty parent dirs (`user/gamification`, `tutorial`). Three URL constants removed from `src/lib/config/api-routes.ts`: `GAMIFICATION_MILESTONE_PRGRESS`, `PLATFORM_CONTRIBUTIONS_KEY`, `PLATFORM_FAIRTEILERS_KEY`. The original Phase 4d plan called for sweeping all read route handlers, but most remain referenced by surviving SWR consumers — history hooks (`use-history-data`, `use-user-history-data`), the preferences trio (`origin/category/company-selection`), `user-settings-wrapper`, `fairteiler-profile-form`, `fairteiler-tags-wrapper`, `tutorial-context`, `preferences-service`, plus mutation cache-invalidation via SWR `mutate()` in `contribution-form` and `contribution-content`. Those route handlers + URL constants stay alive until Phase 5 migrates the remaining call sites; the rest of 4d sweeps up after Phase 5 lands.
